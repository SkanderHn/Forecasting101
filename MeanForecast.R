...
#Fit the model 
fit <- meanf(train , level = c (0,0))

#Generate forecasts 
mean_fct <- forecast(fit, h=5)

#Plot results 
autoplot(mean_fct , ylab = 'Passengers') + scale_x_yearmon() + autolayer(test, series="Data") +
  autolayer(mean_fct$mean, series="Forecasts")
