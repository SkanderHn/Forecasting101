...
#Fit the model 
#High Alpha 
fit <- ses(train , alpha = 0.9 , level = c (0,0))

#Generate forecasts 
ses_fct <- forecast(fit, h=5)

#Plot results 
autoplot(ses_fct , ylab = 'Passengers') + scale_x_yearmon() + autolayer(test, series="Data") +
  autolayer(ses_fct$mean, series="Forecasts")
